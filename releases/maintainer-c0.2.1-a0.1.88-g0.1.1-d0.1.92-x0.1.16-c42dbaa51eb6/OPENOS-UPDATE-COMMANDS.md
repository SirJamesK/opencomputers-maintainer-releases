...
Use this sheet only for release `maintainer-c0.2.1-a0.1.88-g0.1.1-d0.1.92-x0.1.16-c42dbaa51eb6`. The four public artifacts were verified byte-for-byte at the immutable public commit below.

- Release: `maintainer-c0.2.1-a0.1.88-g0.1.1-d0.1.92-x0.1.16-c42dbaa51eb6`
- Public repository: `SirJamesK/opencomputers-maintainer-releases`
- Public commit: `84502fceb2e23d72f04e3ddc9b63a465d4ae03d2`
- Private source commit: `dd4feed74342bd79f39aebb526d8f994ab8ed999`
...
```sh
wget -f https://raw.githubusercontent.com/SirJamesK/opencomputers-maintainer-releases/84502fceb2e23d72f04e3ddc9b63a465d4ae03d2/releases/maintainer-c0.2.1-a0.1.88-g0.1.1-d0.1.92-x0.1.16-c42dbaa51eb6/install.lua /home/oc-release.lua
lua /home/oc-release.lua plan
lua /home/oc-release.lua status
lua /home/oc-release.lua stage https://raw.githubusercontent.com/SirJamesK/opencomputers-maintainer-releases/84502fceb2e23d72f04e3ddc9b63a465d4ae03d2/releases/maintainer-c0.2.1-a0.1.88-g0.1.1-d0.1.92-x0.1.16-c42dbaa51eb6/bundle.ocb NO_BACKUP
lua /home/oc-release.lua status
```
...
11. Do not type a registry identity or topology revision. Configure loads exact
    configured non-Auto fluid identities first, then recorded ordinary-fluid
    evidence, excludes the Auto target set, and probes one candidate per second
    with exact AE2FC `getItemInNetwork` until the first currently positive fluid
    is proven. A configured legacy candidate remains usable when evidence v2 is
    not yet readable, and the screen truthfully marks that search incomplete.
    No plural-fluid or whole-grid read is used.
12. If setup reports that no configured or recorded non-Auto positive fluid
    exists, open Find / Add Craft, start/craft one ordinary fluid outside the
    Auto set while bounded output recording is active, save its evidence, and
    click Configure again. Never invent or type its identity/revision into the
    Dashboard.
...
15. Verify the status strip reads `Fluid Stock 2.8.4 AE2FC` and the detail reads
    `LOCKED ... enrolled=<count> exact fluids` with the current Auto-fluid count.
...
enroll immediately through the existing locked physical profile; they require
no profile rewrite, seed craft, or new generated `AUTO-Rn` epoch. If the canary
itself enters Auto, the profile says `BLOCKED` because its independent positive
witness has been destroyed. Leave automation off and choose a positive canary
outside Maintainer. A host/grid/Fluid Discretizer/isolation topology change
still requires Configure and a newly attested epoch. Never solve either guard
by enabling broad AE2 grid scans. The typed `ae2-workflow
set-fluid-stock-global` command remains emergency compatibility only, not the
normal Dashboard setup path.
