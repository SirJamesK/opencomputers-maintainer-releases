...
local EXPECTED_RELEASE = "maintainer-c0.2.2-a0.1.89-g0.1.1-d0.1.94-x0.1.17-423138f6beeb"
local EXPECTED_BUNDLE_SHA256 = "6bbfee672d5221bb778ef1b5b12564fbc9ea9378c244ddd7006763ecd1c8e24f"
local EXPECTED_FILE_COUNT = 227
local EXPECTED_TARGET_CONTRACT_SHA256 = "9d01f7f824e7214d64633c2ab54fd74606ca32148cc0e978b70afc48372f02a1"
local EXPECTED_PERSISTED_MANIFEST_SHA256 = "722a760db22da57c12d12987faf213401c6b27f5fa9a6dd9d92ab7aec973619d"
...
local POST_INSTALL_CONTRACT = "ae2_gtnh_284_auto_v1"
...
local function requirePlainReleasePaths()
  for _, path in ipairs({
    ...
    INVENTORY_PATH, INVENTORY_PATH .. ".next", LEGACY_LOCK_PATH, LEGACY_LOCK_OWNER_PATH,
    LEGACY_LOCK_OWNER_PATH .. ".next", INHIBIT_PATH, INHIBIT_PATH .. ".next",
...
local function topologyFromState(state)
  ...
  if state.topology == "root" then
    ...
    return {
      ...
      rootMarkerPresent = state.root_marker_present,
    }
  end
...
-- Finalization is already durable before this best-effort request. The
-- installer never owns a physical worker: it persists intent and wakes the
-- already-running AE2 RC broker; boot replays the same idempotent request.
local function requestPostInstallCommissioning()
...
local function postInstallLaunchSuffix()
  local requested, reason = requestPostInstallCommissioning()
  if requested then
    return "; automatic commissioning persisted for the AE2 RC broker"
      .. (reason and (" (" .. tostring(reason):sub(1, 100) .. ")") or "")
  end
  return "; automatic commissioning request deferred to AE2 RC boot: "
    .. tostring(reason):sub(1, 120)
end
...
local function finalizeRelease(offline, backup, verified, confirmation)
  ...
  if state.phase == "finalizing" and state.index == EXPECTED_FILE_COUNT and hasFinalizedReceipt(transactionTopology) then
    ...
    return true, "FINALIZED: completed interrupted metadata cleanup" .. postInstallLaunchSuffix()
  end
