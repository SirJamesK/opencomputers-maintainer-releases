...
Use this sheet only for release `maintainer-c0.1.2-a0.1.79-g0.1.0-d0.1.82-x0.1.13-3fef6d3e9bc2`. The four public artifacts were verified byte-for-byte at the immutable public commit below.

- Release: `maintainer-c0.1.2-a0.1.79-g0.1.0-d0.1.82-x0.1.13-3fef6d3e9bc2`
- Public repository: `SirJamesK/opencomputers-maintainer-releases`
- Public commit: `6e0db545782ef71048e4903a9a2a06484b896f47`
- Private source commit: `39e45553167b86a9e9cc723faf37ee9eb2fb14c2`
...
```sh
wget -f https://raw.githubusercontent.com/SirJamesK/opencomputers-maintainer-releases/6e0db545782ef71048e4903a9a2a06484b896f47/releases/maintainer-c0.1.2-a0.1.79-g0.1.0-d0.1.82-x0.1.13-3fef6d3e9bc2/install.lua /home/oc-release.lua
lua /home/oc-release.lua plan
lua /home/oc-release.lua status
lua /home/oc-release.lua stage https://raw.githubusercontent.com/SirJamesK/opencomputers-maintainer-releases/6e0db545782ef71048e4903a9a2a06484b896f47/releases/maintainer-c0.1.2-a0.1.79-g0.1.0-d0.1.82-x0.1.13-3fef6d3e9bc2/bundle.ocb NO_BACKUP
lua /home/oc-release.lua status
```
...
rebind every craft. New-target Add is config-only; invalid-row discovery/repair
remains quarantined.
...
8. To add a new managed operation whose AE2 crafting pattern already exists,
   open **AE2 -> Targets -> Add Recipe**. Exact Add must open directly without
   stopping the daemon or reading the grid. Select Item or Fluid, enter the
   exact registry/fluid ID, item damage when applicable, and a display label;
   review and save. The receipt must say CONFIG ONLY, MANUAL, OBSERVE, and
   DISARMED. The equivalent shell form is:

   ```sh
   ae2-workflow add-exact fluid <exact-fluid-id> - <display-label...> CONFIRM
   ```

   This binds Maintainer policy to an existing AE2 pattern; it does not create
   that pattern or prove it craftable. Review Flow thresholds and caps before
   enabling Auto, and arm only after stock and recipe status are valid.
9. Run `ae2-workflow show-bind` in a separate shell to check identities. If an
   intended fluid row is a legacy tagged `ae2fc:fluid_drop` / `ae2fc:fluid_packet`
   wrapper, keep it disarmed/unlocked and deliberately supply its exact nested
   Forge fluid identity:

   ```sh
   ae2-workflow rebind <config-index> fluid <exact-fluid-id> - CONFIRM
   ae2-workflow show-bind
   ```

   The repair preserves the same row index, label, intent, thresholds, caps,
   cooldown, and safety gates; it clears stale wrapper authority, performs no
   grid calls, and deliberately leaves the row manual/disarmed. Open Targets,
   select that exact row, and use the Flow button whose text says
   `Shift+A MANUAL (enables Auto)`; do not press it when it says
   `Shift+A Auto ON (toggles OFF)`. Then choose one exact ordinary fluid registry
   name that is always positive and is **not** an auto Maintainer target. `water` is suitable only
...
11. Type that exact canary registry name and press Enter.
12. Type a topology revision such as `GRID-R1` and press Enter. For every later
...
13. Review the preview. Its eligible count must cover every intended ordinary
    auto-fluid identity. Excluded/tagged/invalid rows receive no global authority.
14. Click **Lock Profile**, read the pending attestation, then click
