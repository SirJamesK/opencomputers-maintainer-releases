...
local EXPECTED_RELEASE = "maintainer-c0.2.3-a0.1.103-g0.1.1-d0.1.108-x0.1.21-91430be945de"
local EXPECTED_BUNDLE_SHA256 = "7296136b5e559a5b8b9865da333c07180e24d5a97986e3a4a1a8aa0ee360dd45"
local EXPECTED_FILE_COUNT = 266
local EXPECTED_TARGET_CONTRACT_SHA256 = "6f24cc4239564b6d2dee657284bcca6ab314a02376f72a8e2745427d679d5f5d"
local EXPECTED_PERSISTED_MANIFEST_SHA256 = "7872eee6142b0871675ba48b6ea6ec2cbd61ef6fd159c1ad99a761bc22fe5475"
...
local SHARED_MUTATION_REGISTRY = "__oc_maintainer_release_config_authority_v1"
...
local function startPostInstallBroker()
  ...
  if not statusCalled or type(status) ~= "table"
      or status.workerRunning ~= true or status.workerFresh ~= true
      or status.workerStalled == true then
    local detail = status
    if statusCalled and type(status) == "table" then
      detail = status.workerReason or status.reason or status.state
        or "commissioning broker is not fresh"
    end
...
-- Persist the desired profile while the installer still owns the exact
-- process-local migration lease that protected byte-exact prearchive. Starting
-- the RC broker afterwards makes the durable command precede its executor.
local function requestPostInstallConvergence(commissioning, lease)
  local statusCalled, current = pcall(commissioning.status)
  if not statusCalled or type(current) ~= "table" then
    return false, "commissioning status unavailable before release convergence: "
      .. tostring(current)
  end
  if current.migrationHold == true then
    local serial = tonumber(current.serial)
    if not serial or serial < 1 or serial ~= math.floor(serial) then
      return false, "config migration hold is missing its exact serial"
    end
    -- A pre-existing hold means config conversion has not completed. Preserve
    -- its retained operator intent byte-for-byte; post-FINALIZED
    -- `oc-config-migrate apply` owns conversion and restoration.
    if current.state == "complete" or current.active == true then
      return true, "config migration hold preserved for post-finalize apply",
        { serial = serial }
    end
    return false, "config migration hold is neither active nor complete: "
      .. tostring(current.state or "unknown")
  end
  local options = { source = "release_finalize", migrationLease = lease }
  local called, ok, reason, status = pcall(commissioning.request,
    options)
...
local function verifyRetainedMigrationHold(current, expected)
  if type(expected) ~= "table" then return true end
  local serial = type(current) == "table" and tonumber(current.serial) or nil
  local viable = type(current) == "table"
    and current.migrationHold == true
    and serial == expected.serial
    and (current.state == "complete" or current.active == true)
  if viable then return true end
  return false, "config migration hold changed during RC broker handoff: expected serial "
    .. tostring(expected.serial) .. ", observed serial " .. tostring(serial)
    .. " state " .. tostring(type(current) == "table" and current.state or "unavailable")
end

local function finishPostInstall()
  local loaded, commissioning = pcall(function()
    local bootstrap = require("oc_bootstrap")
    local active, reason = bootstrap.activate()
    if active ~= true then error(tostring(reason or "runtime activation failed"), 0) end
    local verified, verifyReason = bootstrap.verifyActive()
    if verified ~= true then
      error(tostring(verifyReason or "active runtime verification failed"), 0)
    end
    return require("oc.ae2.maintainer_commissioning")
  end)
  if not loaded or type(commissioning) ~= "table"
      or type(commissioning.acquireWorkflowMigration) ~= "function"
      or type(commissioning.releaseWorkflowMigration) ~= "function"
      or type(commissioning.request) ~= "function" then
    return false, "commissioning migration authority is unavailable: "
      .. tostring(commissioning)
  end
  local lease, leaseReason = commissioning.acquireWorkflowMigration()
  if type(lease) ~= "table" then
    return false, "post-install migration authority unavailable: " .. tostring(leaseReason)
  end
  local called, ok, reason = xpcall(function()
    local archiveCalled, archived, archiveReason = pcall(function()
      local migration = require("oc.dashboard.config_migration")
      if type(migration) ~= "table"
          or type(migration.archiveHoldInputs) ~= "function" then
        return false, "config migration archive API is unavailable"
      end
      return migration.archiveHoldInputs({
        migrationLease = lease,
        proveNoLiveControlProcesses = function()
          local running = activeProcess(true)
          if running then
            return false, "active OpenComputers control process blocks deterministic workflow recovery: "
              .. tostring(running)
          end
          return true
        end
    ...
    if not archiveCalled then archived, archiveReason = false, archived end
    if not archived then
      return false, "pre-hold config archive failed: " .. tostring(archiveReason)
    end
    local requested, requestReason, retainedHold = requestPostInstallConvergence(
      commissioning, lease)
    if not requested then return false, requestReason end
    local started, startReason, postStartStatus = startPostInstallBroker()
    if not started then return false, startReason end
    local stable, stableReason = verifyRetainedMigrationHold(
      postStartStatus, retainedHold)
    if not stable then return false, stableReason end
    return true, "; AE2 RC broker online; desired-profile convergence persisted"
      .. (requestReason and (" (" .. tostring(requestReason):sub(1, 100) .. ")") or "")
  end, function(value) return tostring(value) end)
  if not called then ok, reason = false,
    "post-install convergence crashed: " .. tostring(ok) end
  local releaseCalled, released, releaseReason = pcall(
    commissioning.releaseWorkflowMigration, lease)
  if not releaseCalled or released ~= true then
    local detail = tostring(releaseCalled and releaseReason or released)
    if ok then return false, "post-install migration authority release failed: " .. detail end
    reason = tostring(reason) .. "; migration authority release failed: " .. detail
  end
  return ok, reason
end
...
local function acquireProcessAuthority(action)
  ...
  local prior = rawget(process, SHARED_MUTATION_REGISTRY)
  if prior ~= nil then
    checked(type(prior) == "table" and type(prior.record) == "table"
        and type(prior.thread) == "thread", "release/config process authority is mixed")
    checked(coroutine.status(prior.thread) == "dead",
      "another release/config mutation is active; wait for it to exit")
    rawset(process, SHARED_MUTATION_REGISTRY, nil)
  end
  ...
  local shared = { record = current, thread = coroutine.running(), owner = "installer" }
  rawset(process, SHARED_MUTATION_REGISTRY, shared)
  ...
  local scanOk, scanReason = pcall(function()
    ...
    for _, candidate in pairs(process.list) do
      ...
      if candidate ~= current then
        ...
        local peerText = installerProcessText(candidate)
        local named = peerText:find("oc%-release%.lua") ~= nil
          or peerText:find("oc%-config%-migrate") ~= nil
  ...
  if not scanOk then
    ...
    if rawget(process, SHARED_MUTATION_REGISTRY) == shared then
      rawset(process, SHARED_MUTATION_REGISTRY, nil)
    end
    fail(scanReason)
  end
  return current, token, shared, process
end

local function withInstallerAuthority(action, callback, allowLegacyLock)
  local acquired, current, token, shared, process = pcall(acquireProcessAuthority, action)
  ...
  local ownsToken = rawget(current, PROCESS_AUTHORITY_KEY) == token
  local ownsShared = rawget(process, SHARED_MUTATION_REGISTRY) == shared
  local owns = ownsToken and ownsShared
    and shared.thread == coroutine.running() and shared.record == current
  if ownsToken then rawset(current, PROCESS_AUTHORITY_KEY, nil) end
  if ownsShared then
    rawset(process, SHARED_MUTATION_REGISTRY, nil)
  end
...
if (...) == "__module" then return api end
api.entrypoint(...)
