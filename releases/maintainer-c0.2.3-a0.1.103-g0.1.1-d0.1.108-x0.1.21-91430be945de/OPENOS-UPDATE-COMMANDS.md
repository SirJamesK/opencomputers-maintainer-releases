...
Use this sheet only for release `maintainer-c0.2.3-a0.1.103-g0.1.1-d0.1.108-x0.1.21-91430be945de`. The four public artifacts were verified byte-for-byte at the immutable public commit below.

- Release: `maintainer-c0.2.3-a0.1.103-g0.1.1-d0.1.108-x0.1.21-91430be945de`
- Public repository: `SirJamesK/opencomputers-maintainer-releases`
- Public commit: `9ac942dbd2fc2aa2b9b2d678413f5f33acb889cd`
- Private source commit: `86034cb315502fae0ad439d1d0b7e357597b9649`
...
capacity shown by `status`. Use only the reviewed, finalized-only ownership
inventory and one-slot cleanup in section 8; never prune a physical RAID or a
release parent.
...
### Finish a superseded installer already in FINALIZING

If the currently pinned installer already reports `FINALIZING` and fails with
`live config migration process owns daemon desired state`, do not download this installer over
`/home/oc-release.lua`, cancel, roll back, delete transaction
state, or clear AE2 CPUs. CPU occupancy is observational and is not the stop
gate. Start the installed RC broker:

```sh
/etc/rc.d/oc-ae2.lua start
```

If direct execution is unavailable, use `lua /etc/rc.d/oc-ae2.lua start`.
Inspect the retained receipt without opening Dashboard:

```sh
lua -e 'assert(require("oc_bootstrap").activate()); local s=require("oc.ae2.maintainer_commissioning").status(); print("state="..tostring(s.state).." active="..tostring(s.active).." workerRunning="..tostring(s.workerRunning).." reason="..tostring(s.reason))'
```

Wait for `state=complete active=false workerRunning=true`, then finish the exact
old transaction and require `FINALIZED`:
...
Only after that old pinned transaction is `FINALIZED` may this sheet replace
the installer and stage the new immutable bundle.

## 1. Download and stage

```sh
wget -f https://raw.githubusercontent.com/SirJamesK/opencomputers-maintainer-releases/9ac942dbd2fc2aa2b9b2d678413f5f33acb889cd/releases/maintainer-c0.2.3-a0.1.103-g0.1.1-d0.1.108-x0.1.21-91430be945de/install.lua /home/oc-release.lua
lua /home/oc-release.lua plan
lua /home/oc-release.lua status
lua /home/oc-release.lua stage https://raw.githubusercontent.com/SirJamesK/opencomputers-maintainer-releases/9ac942dbd2fc2aa2b9b2d678413f5f33acb889cd/releases/maintainer-c0.2.3-a0.1.103-g0.1.1-d0.1.108-x0.1.21-91430be945de/bundle.ocb NO_BACKUP
lua /home/oc-release.lua status
```
...
=type(require("oc.ae2.maintainer_commissioning_identity").resolve)
=type(require("oc.ae2.maintainer_commissioning").status)
```

Runtime activation must succeed, and all twenty-eight function expressions must
...
Stop, while exposing bounded exact-ID commissioning, Full Automation status,
and stable-ID exception mutation.
...
process-bound migration lease, and archives exact pre-hold workflow and daemon
recovery generations. It then either persists the typed desired-profile request
or preserves an already-active/completed config-migration hold for the
post-finalize apply below; it never recaptures that hold's temporary OFF state.
Only after that durable handoff does it start or reuse
`/etc/rc.d/oc-ae2.lua`, prove the sole RC-owned broker fresh and non-stalled, and recheck that
any retained hold still has the exact same serial and remains active or complete
before releasing the ephemeral lease. Archive, request, malformed/changed hold,
broker-start, or lease-release failure retains recoverable `FINALIZING` state and
the exact `next` command retries it; cleanup occurs only after the handoff
succeeds. A missed wake remains safe because the running bounded watchdog polls
durable intent. Later boot replay accepts only the exact finalized release receipt.

If workflow recovery is already the deterministic `cleanup_previous`,
`restore_previous`, or `promote_staged` generation, this same finalizer may settle
it only while the exact config-migration lease, installer identity,
process exclusivity, and immediate exact-generation recheck still match. An ambiguous,
invalid, or changed generation remains manual recovery; do not delete, rename,
or choose among workflow primary/staged/previous files.
...
## 6. Bring GTNH 2.8.4 Full Automation online with one action
...
visible **Start Full Automation** once. That is the only required automation action: it
durably saves the complete all-media intent before returning, and the AE2
...
phase and then `Full Automation Online`, or a specific blocked reason. A blocked or
drifted persisted Full Automation state is read-only and reports automatic retry;
...
2.8.4 Craft API, auto-configures finite known seed/cell labels locally and
unknown eligible label-only item/fluid rows through bounded exact verification,
commissions fluid-only Fluid Stock authority, installs the versioned
...
Exceptions N`. Every current or future ordinary exact item/fluid or finite known
seed/cell identity is managed automatically even if its local fallback was
created Manual and Disarmed. Those
fallback bits never infer an opt-out: only an explicitly stored stable-workflow-ID
exception excludes an eligible row. The positive Fluid Stock canary may itself
remain automatically maintained under that same exception policy.
In **AE2 -> Flow**, use the Full Auto include/exclude control
to add or remove an explicit row exception. It does
not scan, wait for, or cancel unrelated AE2 CPU jobs. Arbitrary or unsupported
tagged, ambiguous, malformed, and unresolved legacy-fluid-proxy rows remain
ineligible. A selected wrapper repair
...
complete, verify the Daemon strip reports `Full Automation Online`, `Daemon ON`,
`Global Auto ON`, `Full Auto FULL`, and `GUARDS OFF`. The detailed
**AE2 -> Maintainer** status must also report
`Broad Snapshots BLOCKED` and `Craft API 2.8.4 EXACT FILTER`.

Exact-ID configuration scans at most 128 local rows per continuation and makes
zero AE2 calls for rows that already have valid exact identity or a version-pinned
canonical GTNH 2.8.4 seed/cell label. A local mapping yields for 0.1 seconds and
commits through verified stable-workflow-ID compare-and-swap. Each selected
unknown label-only row receives one exact-filter `getCraftables` traversal,
paced no faster than once every two seconds. Every returned handle is checked
against visible label, item/fluid kind, registry name, damage, and tag shape;
only one unique identity may commit through stable-workflow-ID compare-and-swap.
Transport or persistence collisions retry the same row. No result, ambiguity,
malformed output, unsupported tags, or an identity already owned by another
workflow remains a target-local compatibility exception: commissioning never guesses an unknown label
and continues unrelated enrollment. Craftability and
the canonical initial-seed/bare-empty-cell output invariant are rechecked at the
action boundary before any request.
...
It does not bypass exact item/fluid identity, kind-appropriate stock authority,
the active grid-bound fluid-only Fluid Stock proof, durable CAS/readback,
generation and single-owner authority,
...
Stock and Maintainer warm continuations yield for at least 0.4 seconds: five
times the former sustained two-second cadence without increasing the initial
16-row or adaptive 8-32-row burst. Daemon-control schema 2 separately defaults
normal planning and idle polling to 30 seconds. Migration rebases each exact
schema-1 600-second legacy default independently; a non-default schema-1 value
and every explicit schema-2 value remain operator-authoritative.
...
for 2.8.4 Full Automation. Recipe identity comes from structured AE2 CPU
...
1. To add a new managed operation whose AE2 crafting pattern already exists,
   decide its intended Alert Below threshold, Max Request cap, and whether it
   may join Full Automation before binding. Configure an existing matching row
   first when possible. Then open **AE2 -> Targets -> Find / Add Craft**. The page automatically lists
   ...
   **F Look For**, enter a visible label hint, and then start only that intended
   ...
   Select the exact recorded output and choose **Add Exact Target**. The receipt
   reports the raw MANUAL / OBSERVE / DISARMED fallback. Maintainer and Global
   Auto keep their existing ON/OFF states; ordinary Add never invokes
   the repair brake. Active Full Automation immediately enrolls an eligible untagged exact
   item/fluid despite that fallback. Open Flow immediately to set the intended
   threshold/cap; if the target must remain out, use its stable-ID **EXCLUDE**
   action immediately. There is no separate Auto or Arm step. If no
   ...
   ledger. This binds Maintainer policy to an existing AE2 pattern; it does not
   create the AE2 pattern. Exact identity and stock authority still gate any
   request even when Full Automation owns the row.
2. To repair one old, incomplete, or drifted workflow identity, open that
   ...
   executing in external machines. It also revokes Full Automation restart intent
   so cancellation cannot race an automatic restart; to resume later,
   return to AE2 -> Daemon and press Start Full Automation once.
The automatic fluid-only Fluid Stock phase creates a v3 authority marker from one measured
exact-positive AE2FC canary receipt. It tries canonical `water` first, includes
configured Auto fluids, and probes one candidate per second with exact AE2FC
...
If the Daemon page shows **Waiting for Fluid Storage**, connect and power an
AE2FC Fluid Discretizer on the same ME network, give it a channel, and store
water or another configured ordinary fluid. The same setup resumes at its
displayed retry boundary. A Database Upgrade is an item-sample store and cannot
replace this live fluid connection. Setup sends no craft to manufacture this
proof. A terminal **Setup Stopped** state instead requires correcting its
displayed cause and pressing the offered Start action once.
...
An item-shaped AE2FC fluid drop is first resolved through one exact read-only
Craftable output using this pack's `drop of <fluid>` English label. An unresolved
wrapper becomes a signature-bound row exception without a new probe craft.
Only an already-submitted repair retains causal capture to its named CPU,
...
quarantined, the remaining fluid profile commits as `DEGRADED`, Global Auto and
every other eligible non-excepted untagged exact item/fluid converge under Full
Automation, daemon ownership resumes,
...
exact fluids commits `2.8.4 STANDBY / READY FOR FIRST AUTO FLUID`; eligible exact
items still converge through their scalar stock authority. A
...
Later exact-fluid additions, removals, or rebinds enroll immediately through the
existing locked v3 profile; eligible exact-item additions enroll through the
Full Automation overlay. Local fallback-mode changes do not alter membership,
and neither media kind requires a profile rewrite, seed craft, or new epoch.
If the positive fluid witness is depleted or unreadable, affected fluid stock
remains unknown until an exact positive read succeeds. A host, grid, Fluid
...
observe restoration; no repeat Full Automation action is required. Never enable
a broad AE2 scan to bypass this contract. Typed `ae2-workflow
set-fluid-stock-global` remains emergency compatibility only.

## 7. Optional post-commission observation

`Autopilot ONLINE` is the completion proof only when cached Full Automation is
`state=full` with `guardsOff=true`; a legacy fluid-only or missing contract must
render `ATTENTION`. There is no additional smoke-test or enablement gate. Do not
turn the daemon, Global Auto, or managed item/fluid rows OFF merely to test the
release. During normal operation, if an armed eligible row naturally falls below
its threshold, observe that it submits at most one batch, reaches a terminal
lifecycle, and returns to a numeric item or mB value. Recheck that Fluid Stock
...
This release accepts only `auto` or `2.8.4`. A schema-1 scalar/2.9 selection
saved by an older release is decoded as 2.8.4 and canonicalized by the RC-owned
bounded convergence path. A future GTNH version needs a matched release that
defines and verifies its contract; this release rejects an unsupported profile
without changing durable intent.

## 8. Optional post-update external release cleanup

This is optional capacity cleanup for the successful path only. It is not part
of update finalization and it is never valid during `STAGING`, `STAGED`,
`APPLYING`, `APPLIED`, `ROLLING_BACK`, `ROLLED_BACK`, or `FINALIZING`. Start in
a fresh shell after all update verification and run:

```sh
lua /home/oc-release.lua status
```

Proceed only when this exact fresh status reports
`FINALIZED: no active transaction`, `topology: external`, one `runtime address:`
containing the full filesystem UUID, and one `runtime mount:`. If it reports root topology, another
phase, ambiguous/unavailable storage, or any blocker, stop; there is no external
release slot authorized for cleanup. A mount suffix is local and may change
after reboot, so use only the current `runtime mount:` printed on this computer.

Inventory the current runtime and every direct release-slot child without
mutating anything:

```sh
lua
local fs=require("filesystem"); local s,e=require("oc_bootstrap").select(); assert(s,e); assert(s.mountPath~="/","root topology has no external release slots"); local base=fs.concat(s.mountPath,".oc-platform/maintainer/releases"); print("RUNTIME MOUNT "..s.mountPath); print("FULL UUID "..s.address); print("RELEASE ROOT "..base); print("KEEP "..s.release); for name in fs.list(base) do local id=name:gsub("/$",""); print((id==s.release and "KEEP " or "CANDIDATE ")..id) end
```

Press Ctrl+D. The successful `oc_bootstrap.select()` call read-only validates
the selector, current mount's full UUID, host binding, active marker, release
digests, and canonical records. Copy the printed runtime mount exactly, then
replace `<runtime-mount>` in these inspection commands:

```sh
cat /etc/oc/maintainer-runtime.cfg
cat <runtime-mount>/.oc-platform/maintainer/active
ls -la <runtime-mount>/.oc-platform/maintainer/releases/
df
```

The selector and active marker must be byte-for-byte identical canonical
eight-field records. Both must contain the status-reported full filesystem UUID
and `release=maintainer-c0.2.3-a0.1.103-g0.1.1-d0.1.108-x0.1.21-91430be945de`; that release must be the Lua inventory's `KEEP`
entry. Every deletion candidate must be a different, direct child printed as
`CANDIDATE` and confirmed by `ls`. A physically shared RAID may contain unrelated
player or system data; none of it is Maintainer cleanup authority.

For one reviewed inactive candidate, replace both placeholders, reread the
fully expanded path, and run exactly one command:

```sh
rm -r <runtime-mount>/.oc-platform/maintainer/releases/<exact-obsolete-release-id>
```

Never use `*`, `?`, a shell variable, a shortened ID, or a multi-target command.
Never remove the `KEEP`/active release, `/.oc-platform/maintainer/active`,
`/etc/oc/maintainer-runtime.cfg`, the whole `releases` directory, the RAID mount,
`/etc/oc`, or `/var/oc` (including `/var/oc/releases/maintainer`). Remove only
one literal inactive child per reviewed command. After every deletion, rerun the
fresh Lua inventory above and then verify:
...
Status must remain `FINALIZED` on the same full UUID and active release,
`verify-install` must pass, and the active `KEEP` directory must remain. Stop on
the first mismatch instead of deleting another slot.
